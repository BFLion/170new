...
int ME_cost() //calculating the cost using Euclidean Distance heuristic
{
    int me_counter = 1; //ME counter
    int me_cost = 0;    //me_cost

    for (int i = 0; i < currPuzzle.size(); i++)
    {
        for (int j = 0; j < currPuzzle.at(i).size() && me_counter != currPuzzle.size() * currPuzzle.size(); j++)
        {
            if (currPuzzle.at(i).at(j) != to_string(me_counter))
            {
                for (int k = 0; k < currPuzzle.size(); k++)
                {
                    for (int l = 0; l < currPuzzle.at(k).size(); l++)
                    {
                        if (to_string(me_counter) == currPuzzle.at(k).at(l))
                        {
                            me_cost += sqrt(pow((i - k), 2) + pow((j - l), 2)); // Euclidean
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < currPuzzle.size(); i++)
        {
            for (int j = 0; j < currPuzzle.size(); j++)
            {
                if (currPuzzle.at(i).at(j) == "0")
                {
                    ...
                    me_cost += sqrt(pow((sz - i), 2) + pow((sz - j), 2)); // Euclidean
                }
            }
        }
    }
